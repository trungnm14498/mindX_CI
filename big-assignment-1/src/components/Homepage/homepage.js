...
function Homepage() {
    ...
    const incomeValue = incomes.map(income => income.amount).reduce((prevValue, currValue) => prevValue + currValue, 0).toFixed(2);
    const expenseValue = expenses.map(expense => expense.amount).reduce((prevValue, currValue) => prevValue + currValue, 0).toFixed(2);
